...
Each row is a participant. The columns contain the following variables:
...
Now that we have the two estimates above, we can calculate a point-estimate of one of the CDE contrasts, with its confidence interval and a p-value. We use the function below to compare the two fitted estimates, specifying the reference (i.e., the control and low expectancy ($a=0$, $e=0$) combination). We set the contrast to be on the additive scale here. 
...
### High Expectancy ($e=1$)
Now let's estimate the mean counterfactual for the two combinations needed for the contrast:
\[ \mathbb{E}[Y(a=1,e=1) -  Y(a=0,e=1)] .\]

We repeat these same steps with the two other relevant policies for the $e=1$ (high expectancy) setting.
...
Gabriel Loewinger$\ast$, Mats Stensrud, David Yaden, Alex Levis. A Causal Inference Framework for Effect Isolation in Randomized Studies with Unmasking.
